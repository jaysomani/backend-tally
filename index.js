  ...
  */
  app.post('/api/uploadExcel', async (req, res) => {
    try {
      ...
      const fileName = req.body.fileName || 'uploaded_file.xlsx'; // Default filename if none provided
      ...
      await pool.query(`
        CREATE TABLE IF NOT EXISTS ${tempTableName} (
          ...
          transaction_type TEXT CHECK (transaction_type IN ('payment', 'receipt')),
      ...
      // Insert/update mapping in the user_temp_tables table
      await pool.query(
  ...
  */
  app.post('/api/sendToTally', async (req, res) => {
    try {
      ...
      // Transform data to include bank_account and format properly
